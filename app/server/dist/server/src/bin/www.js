...
import logger, { loggerFactory } from '../lib/logger/index.js';
...
function onListening() {
    ...
    if (STREAM_HTML === 'true') {
        ...
        loggerFactory.uiService.info('[EXPRESS] Sending pre-rendered html pages.');
    }
...
function registerShutdownHandlers() {
    ...
    function shutdown(signal) {
        ...
        const forcedExit = setTimeout(() => {
            logger.warn(`⏱️ Force exiting after ${FORCE_SHUTDOWN_MS}ms waiting for shutdown.`);
            exitProcess();
        }, FORCE_SHUTDOWN_MS);
        forcedExit.unref();
