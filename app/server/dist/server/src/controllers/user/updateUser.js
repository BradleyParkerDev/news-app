...
const updateUser = async (req, res) => {
    try {
        ...
        if (result.success) {
            ...
            loggerFactory.user.info(`PUT - /api/user/update-user - userId: ${userUpdates.userId}`);
            return;
        }
        ...
        else if (result.reason === 'missing_current_password' ||
        ...
        loggerFactory.user.error(`PUT - /api/user/update-user - error: ${error instanceof Error ? error.message : String(error)}`);
        const response = {
            success: false,
