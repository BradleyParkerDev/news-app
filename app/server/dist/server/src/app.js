...
const CLIENT_ORIGIN = NODE_ENV === 'production'
    ? 'https://pernapp.bradleyparker.dev'
    : 'http://localhost:4001';
const openapi = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), 'openapi.json'), 'utf-8'));
/**
 ...
 */
app.use('/fonts', express.static(path.resolve(process.cwd(), 'app/client/public/fonts')));
app.use('/assets', express.static(path.resolve(process.cwd(), 'app/client/public/assets')));
if (NODE_ENV === 'production') {
    app.use('/static', express.static(path.resolve(process.cwd(), 'app/client/dist/static'), {
        maxAge: '1y',
        immutable: true,
    }));
}
