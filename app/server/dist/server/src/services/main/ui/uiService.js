...
import { reactHelper } from '../../../services/helpers/index.js';
...
import { UserTheme } from '../../../database/schemas/UserThemes.js';
import { db } from '../../../database/db.js';
import { eq } from 'drizzle-orm';
export const createUiService = (req, res) => {
    ...
    const uiService = {
        ...
        async handlePageRendering() {
            ...
            const pathToLog = this.req.originalUrl.replace(/\/+($|\?)/, '$1') || '/';
            if (!ignored.has(pathToLog)) {
                const sessionId = this.req.body.sessionId ?? 'unknown';
                const userId = this.req.body.userId ?? 'guest';
                loggerFactory.index.info(`GET - ${pathToLog} - sessionId: ${sessionId} - userId: ${userId}`);
            }
        },
        async getUserTheme() {
