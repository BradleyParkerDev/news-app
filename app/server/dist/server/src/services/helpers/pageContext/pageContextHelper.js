...
import { setAppName, setTheme, loadCurrentPageState, } from '../../../../../shared/redux/slices/ui/uiSlice.js';
...
export const createPageContextHelper = (req, res) => {
    ...
    const pageContextHelper = {
        ...
        async loadAppDataIntoRedux(userTheme) {
            ...
            if (userId) {
                ...
                if (user) {
                    const userProfileImage = await imageHelper.getUserProfileImage(userId);
                    this.store.dispatch(setAuth({ isAuth: true }));
                    this.store.dispatch(setUser({ userData: user }));
                    this.store.dispatch(setUserProfileImage({
            ...
            this.store.dispatch(loadCurrentPageState({
                currentPage: {
                    path: this.path,
                    content: pageContent,
                    isLoading: false,
            ...
            loggerFactory.uiService.info(`[REDUX] loadAppDataIntoRedux - page: ${this.path}`);
        },
        // Return content based on the current route path.
        async getPageContent(pathOverride) {
            const requestedPath = typeof pathOverride === 'string' && pathOverride.length > 0
                ? pathOverride
