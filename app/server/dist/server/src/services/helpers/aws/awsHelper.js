...
export const awsHelper = {
    async uploadObjectToS3Bucket({ buffer, mimetype, originalname, prefix = 'images', storageId, }) {
    ...
    async deleteObjectInS3Bucket({ key, }) {
        try {
            await s3Client.send(new DeleteObjectCommand({
                Bucket: bucketName,
                Key: key,
            }));
            await waitUntilObjectNotExists({ client: s3Client, maxWaitTime: 30 }, { Bucket: bucketName, Key: key });
            return {
                success: true,
            ...
            if (error instanceof S3ServiceException) {
                console.error(`S3 delete failed: ${error.name} - ${error.message}`);
                return {
                    success: false,
