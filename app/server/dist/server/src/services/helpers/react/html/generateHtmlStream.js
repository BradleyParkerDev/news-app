...
export const generateHtmlStream = async (streamData) => {
    const { res, app, routerContext, appState } = streamData;
    let template = fs.readFileSync(path.resolve('app/server/src/services/helpers/react/ssr.html'), 'utf8');
    const scripts = generateScripts(routerContext, appState);
    template = template.replace('%%CSS%%', scripts.css);
    template = template.replace('%%JS%%', scripts.js);
    template = template.replace('%%VITE_DEV_SERVER%%', scripts.viteDevServer);
    template = template.replace('%%HYDRATION_SCRIPTS%%', scripts.hydrationScripts);
    template = template.replace('%%SSR_HTML%%', '');
    const marker = '<div id="root">';
    const idx = template.indexOf(marker);
    if (idx === -1) {
    ...
    const { pipe, abort } = renderToPipeableStream(app, {
        ...
        onError(err) {
            loggerFactory.uiService.error(`[SSR][STREAM][ERROR] ${err instanceof Error ? err.message : String(err)}`);
            if (!res.headersSent) {
                res.status(HTTPStatus.INTERNAL_SERVER_ERROR).send('SSR Error');
            }
        },
    });
    res.on('close', () => abort());
