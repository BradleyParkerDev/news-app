...
export const generateHtml = (ssrHtmlFromReact, routerContext, appState) => {
    ...
    let htmlTemplate = fs.readFileSync(path.resolve('app/server/src/services/helpers/react/ssr.html'), 'utf8');
    // 3. Insert React ssr
    ...
    htmlTemplate = htmlTemplate.replace('%%VITE_DEV_SERVER%%', scripts.viteDevServer);
    htmlTemplate = htmlTemplate.replace('%%HYDRATION_SCRIPTS%%', scripts.hydrationScripts);
    return htmlTemplate;
};
